...
import { RecordTypeBadge, EventClassBadge, FormatBadge, ConfidenceBadge } from './Badges';
    ...
    return (
      <div className="border border-ink-100 rounded-lg p-4 bg-ink-50/50 relative overflow-hidden">
        ...
        <div className="opacity-30">
          ...
          <p className="text-xs text-ink-500">{record.date}</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="border border-ink-100 rounded-lg p-5 bg-white">
      ...
      <div className="flex items-center gap-2 text-xs text-ink-500 mb-3">
        ...
        <span>{record.date}</span>
        <span className="text-ink-200">|</span>
        <span>{record.territory.replace('_', ' ')}</span>
      </div>
      ...
      <div className="text-xs text-ink-600 mb-4">
        <span className="font-semibold text-ink-800">Professional action:</span> {record.professionalAction}
      </div>
      <Link
        to={`/deals/${record.id}`}
