...
export function BriefingPage() {
  const { briefing: issue, records, buyers } = useData();

  const buyerId = issue.buyerToWatch.buyerMatch || buyers.find((b) =>
    b.name.toLowerCase().includes(issue.buyerToWatch.name.toLowerCase()),
  )?.id;

  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 py-10 sm:py-14">
      <SEO
        title={`${issue.headline} | The Pickup`}
        description={issue.deck || 'The weekly intelligence briefing on entertainment acquisitions and buyer mandates.'}
      />
        ...
        <section className="py-9 border-b border-ink-200">
          ...
          <div className="mt-2 divide-y divide-ink-200">
            {issue.moneyMoves.map((story) => {
              const recordId = story.recordMatch
                ? (records.find(r => r.id === story.recordMatch) ? story.recordMatch : undefined)
                : undefined;
              return (
      ...
      {issue.legacyCrossovers && issue.legacyCrossovers.length > 0 && (
        <section className="py-9 border-b border-ink-200">
          <SectionTitle title="Legacy Crossovers" eyebrow="Traditional buyers entering new formats" />
          <div className="mt-2 divide-y divide-ink-200">
            {issue.legacyCrossovers.map((story) => {
              const recordId = story.recordMatch
                ? (records.find(r => r.id === story.recordMatch) ? story.recordMatch : undefined)
                : undefined;
              return (
                  ...
                  <div className="flex flex-wrap items-center justify-between gap-3 mt-5">
                    <p className="text-xs text-ink-500">
                      ...
                      {story.sources.map((source, index) => (
                        <span key={source.url}>
                          {index > 0 && ' · '}
                          <a href={source.url} target="_blank" rel="noopener noreferrer" className="underline decoration-ink-300 underline-offset-2 hover:text-ink-800">
                            {source.name}
                          </a>
                        </span>
                      ))}
                    ...
                    {recordId && (
                      <Link to={`/deals/${recordId}`} className="inline-flex items-center gap-1 text-xs font-semibold text-inkred hover:text-inkred-700">
                        Full record <ArrowRight size={12} />
                      </Link>
                    )}
                  </div>
      ...
      {issue.mandatesForming.length > 0 && (
        <section className="py-9 border-b border-ink-200">
