  ...
  return (
    <div className="max-w-3xl mx-auto px-4 sm:px-6 py-8">
      ...
      <header className="mt-6 mb-8">
        ...
        <h1 className="text-xl sm:text-2xl font-bold text-ink-900 leading-tight mb-2">
          {issue.headline}
        </h1>
      ...
      <section className="mb-8">
        ...
        <p className="text-sm text-ink-700 leading-relaxed mt-3 border-l-2 border-burgundy-300 pl-3">
          {issue.signalThisWeek}
        </p>
      </section>

      {/* Money Moves — confirmed deals only */}
      <section className="mb-8">
          ...
          <div className="mt-3">
            {legacyCrossoverRecords.map(record => record && (
              <div key={record.id} className="py-3">
                ...
                <Link to={`/deals/${record.id}`} className="inline-flex items-center gap-1 mt-1.5 text-xs font-medium text-burgundy-700 hover:text-burgundy-900">
      ...
      {buyerToWatch && (
        <section className="mb-8">
          ...
          <div className="mt-3 py-3 border-t border-ink-100">
            ...
            <Link to={`/buyers/${buyerToWatch.id}`} className="inline-flex items-center gap-1 mt-2 text-xs font-medium text-burgundy-700 hover:text-burgundy-900">
        ...
        <div className="mt-3 divide-y divide-ink-100">
          {issue.quickCuts.slice(0, 5).map((item, i) => (
            <div key={i} className="py-2.5 flex items-start justify-between gap-3">
              ...
              <a href={item.sourceUrl} target="_blank" rel="noopener noreferrer" className="shrink-0 text-xs text-ink-400 hover:text-ink-600 underline">
                Source
              </a>
            </div>
          ))}
...
function SectionHeader({ icon, title }: { icon: React.ReactNode; title: string }) {
  return (
    <div className="flex items-center gap-2 pb-1.5 border-b border-ink-100">
      <span className="text-burgundy-600">{icon}</span>
      <h2 className="text-xs font-bold text-ink-900 uppercase tracking-wide">{title}</h2>
    </div>
  );
}
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          {record.sources.map((source, i) => (
            <a key={i} href={source.url} target="_blank" rel="noopener noreferrer" className="text-xs text-ink-500 hover:text-ink-700 underline">
              {source.name} ({source.readTime})
            </a>
          ))}
        </div>
        <Link to={`/deals/${record.id}`} className="text-xs font-medium text-burgundy-700 hover:text-burgundy-900">
          Full record
        </Link>
      </div>
    </article>
...
function MandateBlock({ mandate }: { mandate: { signalType: string; confidence: Confidence; explanation: string; whyItMatters: string; evidenceUrl: string } }) {
  ...
  return (
    <div className={`border-l-2 ${borderColor[mandate.confidence]} pl-3 py-2`}>
      ...
      <a href={mandate.evidenceUrl} target="_blank" rel="noopener noreferrer" className="text-xs text-ink-400 hover:text-ink-600 underline mt-0.5 inline-block">
        Evidence
      </a>
    </div>
  );
